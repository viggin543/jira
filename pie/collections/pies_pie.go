...
func (ss Pies) All(fn func(value Pie) bool) bool {
...
func (ss Pies) Any(fn func(value Pie) bool) bool {
	for _, value := range ss {
		if fn(value) {
			return true
		}
...
func (ss Pies) Append(elements ...Pie) Pies {
	// Copy ss, to make sure no memory is overlapping between input and
	// output. See issue #97.
	result := append(Pies{}, ss...)
...
func (ss Pies) Bottom(n int) (top Pies) {
...
func (ss Pies) Contains(lookingFor Pie) bool {
	for _, s := range ss {
		if lookingFor == s {
			return true
		}
...
func (ss Pies) Diff(against Pies) (added, removed Pies) {
	...
	diffOneWay := func(ss1, ss2raw Pies) (result Pies) {
		ss2 := make(Pies, len(ss2raw))
		copy(ss2, ss2raw)
...
// if n < 0 it'll return empty slice.
func (ss Pies) DropTop(n int) (drop Pies) {
	if n < 0 || n >= len(ss) {
		return
	}
...
//   })
//
func (ss Pies) Each(fn func(Pie)) Pies {
	for _, s := range ss {
		fn(s)
	}
...
func (ss Pies) Equals(rhs Pies) bool {
	if len(ss) != len(rhs) {
		return false
	}
...
//
// It is acceptable to provide zero arguments.
func (ss Pies) Extend(slices ...Pies) (ss2 Pies) {
	ss2 = ss
...
func (ss Pies) Filter(condition func(Pie) bool) (ss2 Pies) {
	for _, s := range ss {
		if condition(s) {
			ss2 = append(ss2, s)
		}
...
func (ss Pies) FilterNot(condition func(Pie) bool) (ss2 Pies) {
	for _, s := range ss {
		if !condition(s) {
			ss2 = append(ss2, s)
		}
...
func (ss Pies) FindFirstUsing(fn func(value Pie) bool) int {
...
func (ss Pies) First() Pie {
	return ss.FirstOr(Pie{})
}

// FirstOr returns the first element or a default value if there are no
// elements.
func (ss Pies) FirstOr(defaultValue Pie) Pie {
	if len(ss) == 0 {
		return defaultValue
	}
...
func (ss Pies) Float64s() pie.Float64s {
	l := len(ss)
...
func (ss Pies) Join(glue string) (s string) {
	var slice interface{} = []Pie(ss)
...
func (ss Pies) JSONBytesIndent(prefix, indent string) []byte {
	if ss == nil {
		return []byte("[]")
	}
...
// One important thing to note is that it will treat a nil slice as an empty
// slice to ensure that the JSON value return is always an array.
func (ss Pies) JSONString() string {
	if ss == nil {
		return "[]"
	}
...
// slice to ensure that the JSON value return is always an array. See
// json.MarshalIndent for details.
func (ss Pies) JSONStringIndent(prefix, indent string) string {
	if ss == nil {
		return "[]"
	}
...
func (ss Pies) Last() Pie {
	return ss.LastOr(Pie{})
}

// LastOr returns the last element or a default value if there are no elements.
func (ss Pies) LastOr(defaultValue Pie) Pie {
	if len(ss) == 0 {
		return defaultValue
	}
...
func (ss Pies) Map(fn func(Pie) Pie) (ss2 Pies) {
	if ss == nil {
		return nil
	}
...
func (ss Pies) Mode() Pies {
	if len(ss) == 0 {
		return nil
	}
	values := make(map[Pie]int, 0)
	for _, s := range ss {
		values[s]++
	}
...
func (ss Pies) Random(source rand.Source) Pie {
	n := len(ss)
...
func (ss Pies) Send(ctx context.Context, ch chan<- Pie) Pies {
	for i, s := range ss {
		select {
		case <-ctx.Done():
			return ss[:i]
...
func (ss Pies) SequenceUsing(creator func(int) Pie, params ...int) Pies {
	var seq = func(min, max, step int) (seq Pies) {
		lenght := int(util.Round(float64(max-min) / float64(step)))
		if lenght < 1 {
			return
		}

		seq = make(Pies, lenght)
...
func (ss Pies) Shift() (Pie, Pies) {
	return ss.First(), ss.DropTop(1)
}

// Shuffle returns shuffled slice by your rand.Source
func (ss Pies) Shuffle(source rand.Source) Pies {
	n := len(ss)
...
func (ss Pies) SortStableUsing(less func(a, b Pie) bool) Pies {
	// Avoid the allocation. If there is one element or less it is already
	// sorted.
	if len(ss) < 2 {
		return ss
...
// slice returned will be reallocated as to not modify the input slice.
func (ss Pies) SortUsing(less func(a, b Pie) bool) Pies {
	// Avoid the allocation. If there is one element or less it is already
	// sorted.
	if len(ss) < 2 {
		return ss
...
//   fmt.Sprintf("%v")
//
func (ss Pies) Strings() pie.Strings {
	l := len(ss)
...
func (ss Pies) SubSlice(start int, end int) (subSlice Pies) {
	if start < 0 || end < 0 {
		return
	}
	...
	if start < length {
		if end <= length {
			...
			zeroArray := make([]Pie, end-length)
			subSlice = ss[start:length].Append(zeroArray[:]...)
		}
	} else {
		zeroArray := make([]Pie, end-start)
		subSlice = zeroArray[:]
	}
...
// if the slice has less elements then n that'll return all elements
// if n < 0 it'll return empty slice.
func (ss Pies) Top(n int) (top Pies) {
...
func (ss Pies) StringsUsing(transform func(Pie) string) pie.Strings {
	l := len(ss)
...
func (ss Pies) Unshift(elements ...Pie) (unshift Pies) {
	unshift = append(Pies{}, elements...)
